...
public class EmailAtendenteDAO implements EmailDAO {

    @Override
    public void inserir(Connection con, Email email, int idPessoa) throws Exception {
        String sql = "INSERT INTO EmailAtendente (enderecoEmail, idAtendente) VALUES (?, ?) RETURNING idEmail";
    ...
    public List<Email> buscarPorPessoa(Connection con, int idPessoa) throws Exception {
        String sql = "SELECT * FROM EmailAtendente WHERE idAtendente = ?";
